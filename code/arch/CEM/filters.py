"""
filters.py (08-06-21)
https://github.com/victorca25/BasicSR/blob/c594cada9422f6f3447fbeb2b2e21e4407ab1188/codes/dataops/filters.py
...
def get_log_kernel2d(kernel_size=5, sigma=None, dim=2):  # sigma=0.6; kernel_size=5
    ...
    if not kernel_size and sigma:
        ...
        kernel_size = [
            kernel_size
        ] * dim  # note: should it be [kernel_size] or [kernel_size-1]? look below
...
def get_laplacian_kernel_3x3(alt=False) -> torch.Tensor:
    ...
    """
    if alt:
        return torch.tensor([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]])
    else:
        return torch.tensor(
            [
                [0, 1, 0],
...
def get_4dim_image_gradients(image):
    ...
    right = F.pad(image, [0, 1, 0, 0])[
        ..., :, 1:
    ]  # adds one pixel pad to the right and removes one pixel from the left
    bottom = F.pad(image, [0, 0, 0, 1])[
        ..., 1:, :
    ]  # adds one pixel pad to the bottom and removes one pixel from the top
    botright = F.pad(image, [0, 1, 0, 1])[
        ..., 1:, 1:
...
def guided_filter(
    ...
    if not isinstance(box_kernel, torch.Tensor):
        ...
        if not ks:
            if r:
                ...
                raise ValueError(
                    "Either kernel size (ks) or radius (r) for the window are required."
                )
    ...
    cov_xy = (
        filter2D(x * y, box_kernel) / N
    ) - mean_x * mean_y  # Corrected from: - mean_x - mean_y
    var_x = (
        filter2D(x * x, box_kernel) / N
    ) - mean_x * mean_x  # Corrected from: - mean_x - mean_x
