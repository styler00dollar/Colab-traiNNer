...
class EdgeSRModel(nn.Module):
    ...
    def forward(self, lr, lr_edges):
        # edge
        # inputting rgb data
        lr_scaled = F.interpolate(lr, scale_factor=4) # NN upscaling rgb LR to match HR size
        ...
        lr_edges_scaled = F.interpolate(lr_edges, scale_factor=4) # NN upscaling lr canny to match HR size
        inputs = torch.cat((lr_scaled, lr_edges_scaled), dim=1)

        edge_gen = self.EdgeGenerator(inputs)  # in: [rgb(3) + edge(1)] / out: edge(1) (estimaged edges for HR image)

        # inpaint
        inputs = torch.cat((lr_scaled, edge_gen), dim=1)

        outputs = self.SRGenerator(inputs)  # in: [rgb(3) + output(1)] / out: rgb(3) (SR image)
        return outputs
