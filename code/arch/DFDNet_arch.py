...
class UNetDictFace(pl.LightningModule):
    def __init__(self, ngf=64):
        ...
        for j, i in enumerate(Parts):
            f_256 = torch.from_numpy(
                np.load(
                    os.path.join(dictionary_path, "{}_256_center.npy".format(i)),
                    allow_pickle=True,
                )
            )
            ...
            f_128 = torch.from_numpy(
                np.load(
                    os.path.join(dictionary_path, "{}_128_center.npy".format(i)),
                    allow_pickle=True,
                )
            )
            ...
            f_64 = torch.from_numpy(
                np.load(
                    os.path.join(dictionary_path, "{}_64_center.npy".format(i)),
                    allow_pickle=True,
                )
            )
            ...
            f_32 = torch.from_numpy(
                np.load(
                    os.path.join(dictionary_path, "{}_32_center.npy".format(i)),
                    allow_pickle=True,
                )
            )
    ...
    def forward(self, input, part_locations):
        ...
        for i, f_size in enumerate(self.feature_sizes):
            ...
            LE_feature = cur_feature[
                :, :, le_location[1] : le_location[3], le_location[0] : le_location[2]
            ].clone()
            RE_feature = cur_feature[
                :, :, re_location[1] : re_location[3], re_location[0] : re_location[2]
            ].clone()
            NO_feature = cur_feature[
                :, :, no_location[1] : no_location[3], no_location[0] : no_location[2]
            ].clone()
            MO_feature = cur_feature[
                :, :, mo_location[1] : mo_location[3], mo_location[0] : mo_location[2]
            ].clone()

            # resize
            LE_feature_resize = F.interpolate(
                LE_feature,
                (LE_Dict_feature.size(2), LE_Dict_feature.size(3)),
                mode="bilinear",
                align_corners=False,
            )
            RE_feature_resize = F.interpolate(
                RE_feature,
                (RE_Dict_feature.size(2), RE_Dict_feature.size(3)),
                mode="bilinear",
                align_corners=False,
            )
            NO_feature_resize = F.interpolate(
                NO_feature,
                (NO_Dict_feature.size(2), NO_Dict_feature.size(3)),
                mode="bilinear",
                align_corners=False,
            )
            MO_feature_resize = F.interpolate(
                MO_feature,
                (MO_Dict_feature.size(2), MO_Dict_feature.size(3)),
                mode="bilinear",
                align_corners=False,
            )

            LE_Dict_feature_norm = adaptive_instance_normalization_4D(
                LE_Dict_feature, LE_feature_resize
            )
            RE_Dict_feature_norm = adaptive_instance_normalization_4D(
                RE_Dict_feature, RE_feature_resize
            )
            NO_Dict_feature_norm = adaptive_instance_normalization_4D(
                NO_Dict_feature, NO_feature_resize
            )
            MO_Dict_feature_norm = adaptive_instance_normalization_4D(
                MO_Dict_feature, MO_feature_resize
            )
            ...
            LE_Swap_feature = F.interpolate(
                LE_Dict_feature_norm[LE_index : LE_index + 1],
                (LE_feature.size(2), LE_feature.size(3)),
            )

            LE_Attention = getattr(self, "le_" + str(f_size))(
                LE_Swap_feature - LE_feature
            )
            ...
            RE_Swap_feature = F.interpolate(
                RE_Dict_feature_norm[RE_index : RE_index + 1],
                (RE_feature.size(2), RE_feature.size(3)),
            )

            RE_Attention = getattr(self, "re_" + str(f_size))(
                RE_Swap_feature - RE_feature
            )
            ...
            NO_Swap_feature = F.interpolate(
                NO_Dict_feature_norm[NO_index : NO_index + 1],
                (NO_feature.size(2), NO_feature.size(3)),
            )

            NO_Attention = getattr(self, "no_" + str(f_size))(
                NO_Swap_feature - NO_feature
            )
            ...
            MO_Swap_feature = F.interpolate(
                MO_Dict_feature_norm[MO_index : MO_index + 1],
                (MO_feature.size(2), MO_feature.size(3)),
            )

            MO_Attention = getattr(self, "mo_" + str(f_size))(
                MO_Swap_feature - MO_feature
            )
            MO_Att_feature = MO_Attention * MO_Swap_feature

            update_feature[
                :, :, le_location[1] : le_location[3], le_location[0] : le_location[2]
            ] = (LE_Att_feature + LE_feature)
            update_feature[
                :, :, re_location[1] : re_location[3], re_location[0] : re_location[2]
            ] = (RE_Att_feature + RE_feature)
            update_feature[
                :, :, no_location[1] : no_location[3], no_location[0] : no_location[2]
            ] = (NO_Att_feature + NO_feature)
            update_feature[
                :, :, mo_location[1] : mo_location[3], mo_location[0] : mo_location[2]
            ] = (MO_Att_feature + MO_feature)
