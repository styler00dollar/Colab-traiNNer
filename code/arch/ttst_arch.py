# https://github.com/XY-boy/TTST/blob/main/model_archs/TTST_arc.py
import math
import torch
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        mask1 = torch.zeros(
            b, self.num_heads, C, C, device=x.device, requires_grad=False
        )
        mask2 = torch.zeros(
            b, self.num_heads, C, C, device=x.device, requires_grad=False
        )
        ...
        mask1.scatter_(-1, index, 1.0)
        attn1 = torch.where(mask1 > 0, attn, torch.full_like(attn, float("-inf")))

        # print(111, mask1.scatter_(-1, index, 1.))

        index = torch.topk(attn, k=int(C * 2 / 3), dim=-1, largest=True)[1]
        mask2.scatter_(-1, index, 1.0)
        attn2 = torch.where(mask2 > 0, attn, torch.full_like(attn, float("-inf")))

        index = torch.topk(attn, k=int(C * 3 / 4), dim=-1, largest=True)[1]
        mask3.scatter_(-1, index, 1.0)
        attn3 = torch.where(mask3 > 0, attn, torch.full_like(attn, float("-inf")))

        index = torch.topk(attn, k=int(C * 4 / 5), dim=-1, largest=True)[1]
        mask4.scatter_(-1, index, 1.0)
