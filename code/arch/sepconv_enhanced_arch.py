    ...
    def __init__(self):
        ...
        self.load_state_dict(
            torch.hub.load_state_dict_from_url(
                url="http://content.sniklaus.com/resepconv/network-"
                + arguments_strModel
                + ".pytorch",
...
if __name__ == "__main__":
    if arguments_strOut.split(".")[-1] in ["bmp", "jpg", "jpeg", "png"]:
        ...
        PIL.Image.fromarray(
            (
                tenOutput.clip(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, ::-1] * 255.0
            ).astype(numpy.uint8)
        ...
        with moviepy.video.io.ffmpeg_writer.FFMPEG_VideoWriter(
            filename=arguments_strOut,
            size=(intWidth, intHeight),
            fps=objVideoreader.fps,
        ) as objVideowriter:
            for npyFrame in objVideoreader.iter_frames():
                tenFrames[4] = torch.FloatTensor(
                    numpy.ascontiguousarray(
                        npyFrame[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32)
                        * (1.0 / 255.0)
                    )
                )
