...
class GridNet(nn.Module):
    ...
    def forward(self, x, x1, x2, x3, x4):
        X00 = (
            self.residual_model_head(x.detach().clone())
            + self.residual_model_head1(x1.detach().clone())
            + self.residual_model_head4(x4.detach().clone())
        )  # ---   182 ~ 185
...
class GMFSS_union(nn.Module):
    def __init__(self, partial_fp16=False):
        super(GMFSS_union, self).__init__()
        ...
        self.metricnet.load_state_dict(
            torch.load(
                "/home/user/Schreibtisch/Colab-traiNNer/train/GMFSS_union_metric_vanilla.pkl"
            )
        )
        self.fusionnet.load_state_dict(
            torch.load(
                "/home/user/Schreibtisch/Colab-traiNNer/train/GMFSS_union_fusionnet_vanilla.pkl"
            )
        )
        self.flownet.load_state_dict(
            torch.load(
                "/home/user/Schreibtisch/Colab-traiNNer/train/GMFupSS_flownet.pkl"
            )
        )
        self.ifnet.load_state_dict(
            torch.load("/home/user/Schreibtisch/Colab-traiNNer/train/rife46.pth")
        )
        """
    ...
    def forward(self, I0, I1, timestep=0.5, scale=1.0):
        # padding frames
        n, c, h, w = I0.shape
        # print(n, c, h, w)
        ...
        I0 = F.pad(I0, padding)
        I1 = F.pad(I1, padding)
        flow01, flow10, metric0, metric1, feat_ext0, feat_ext1 = self.reuse(
            I0, I1, scale=1.0
        )
