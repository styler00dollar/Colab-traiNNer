"""
https://github.com/tsingqgou/misf
kpn/network.py
src/networks.py
"""
...
class Basic(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.channel_att:
            ...
            fm_pool = torch.cat(
                [F.adaptive_avg_pool2d(fm, (1, 1)), F.adaptive_max_pool2d(fm, (1, 1))],
                dim=1,
            )
...
class KPN(nn.Module):
    def __init__(
        ...
        # 2~5层都是均值池化+3层卷积
        self.conv1 = Basic(
            in_channel, 64, channel_att=False, spatial_att=False
        )  # 256*256
        self.conv2 = Basic(64, 128, channel_att=False, spatial_att=False)  # 128*128
        self.conv3 = Basic(
            128 + 128, 256, channel_att=False, spatial_att=False
        )  # 64*64
        ...
        self.conv7 = Basic(
            256 + 512, 256, channel_att=channel_att, spatial_att=spatial_att
        )
        self.conv8 = Basic(
            256 + 256, 128, channel_att=channel_att, spatial_att=spatial_att
        )
        self.conv9 = Basic(
            128 + 64, 64, channel_att=channel_att, spatial_att=spatial_att
        )
    ...
    def forward(self, data_with_est, x):
        ...
        conv9 = self.conv9(
            torch.cat(
                [conv1, F.interpolate(conv8, scale_factor=2, mode=self.upMode)], dim=1
            )
        )  # 64*256*256
...
class KernelConv(nn.Module):
    ...
    def _sep_conv_core(self, core, batch_size, N, color, height, width):
        ...
        for K in self.kernel_size:
            t1 = core_1[:, :, cur : cur + K, ...].view(
                batch_size, N, K, 1, 3, height, width
            )
            t2 = core_2[:, :, cur : cur + K, ...].view(
                batch_size, N, 1, K, 3, height, width
            )
    ...
    def forward(self, frames, core, white_level=1.0, rate=1):
        ...
        for index, K in enumerate(kernel):
            if not img_stack:
                ...
                frame_pad = F.pad(
                    frames, [padding_num, padding_num, padding_num, padding_num]
                )
                for i in range(0, K):
                    for j in range(0, K):
                        img_stack.append(
...
class LossBasic(nn.Module):
    ...
    def forward(self, pred, ground_truth):
        return self.l2_loss(pred, ground_truth) + self.l1_loss(
            self.gradient(pred), self.gradient(ground_truth)
        )
...
class MISF(nn.Module):
    def __init__(self, residual_blocks=8, use_spectral_norm=True):
        super(MISF, self).__init__()
        self.EdgeGenerator = EdgeGenerator(
            residual_blocks=residual_blocks, use_spectral_norm=use_spectral_norm
        )
        self.InpaintGenerator = InpaintGenerator(residual_blocks=residual_blocks)

    def forward(self, input, edge, grayscale, mask):
        edges_masked = edge * mask
        grayscale_masked = grayscale * mask
        inputs = torch.cat([grayscale_masked, edges_masked, mask], dim=1)

        edge = self.EdgeGenerator(inputs)
        out = self.InpaintGenerator(torch.cat([input, edge, mask], dim=1))
        return out, edge
