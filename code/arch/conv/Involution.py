...
class Involution(nn.Module):
    ...
    def forward(self, inputs):
        ...
        weight = self.conv2(
            self.relu(self.bn(self.conv1(self.avgpool(inputs))))
        )  # (bs,G*K*K,H//stride,W//stride)
        b, c, h, w = weight.shape
        weight = weight.reshape(
            b, self.group, self.kernel_size * self.kernel_size, h, w
        ).unsqueeze(
            2
        )  # (bs,G,1,K*K,H//stride,W//stride)
        ...
        x_unfold = x_unfold.reshape(
            ...
            H // self.stride,
            W // self.stride,
        )  # (bs,G,G//C,K*K,H//stride,W//stride)

        out = (x_unfold * weight).sum(dim=3)  # (bs,G,G//C,1,H//stride,W//stride)
        out = out.reshape(
            B, C, H // self.stride, W // self.stride
