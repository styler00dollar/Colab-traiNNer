...
from loss import perceptual_loss as util
    ...
    def forward(self, in0, in1, retPerLayer=False):
        ...
        if self.lpips:
            if self.spatial:
                ...
                res = [
                    upsample(self.lins[kk].model(diffs[kk]), out_size=in0.shape[2:])
                    for kk in range(self.L)
                ]
            else:
                res = [
            ...
            if self.spatial:
                ...
                res = [
                    upsample(diffs[kk].sum(dim=1, keepdim=True), out_size=in0.shape[2:])
                    for kk in range(self.L)
                ]
            else:
                res = [
