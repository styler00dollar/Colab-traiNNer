...
from basicsr.utils import FileClient, get_root_logger, imfrombytes, img2tensor, DiffJPEG, USMSharp
...
class RealESRGANDataset(pl.LightningDataModule):
    ...
    def __getitem__(self, index):
        ...
        if np.random.uniform() < 0.5:
            ...
            mode = random.choice(['area', 'bilinear', 'bicubic'])
            out = F.interpolate(out, size=(ori_h // self.config['scale'], ori_w // self.config['scale']), mode=mode)
        ...
        return 0, self.lq.squeeze(0), self.gt_usm.squeeze(0)
