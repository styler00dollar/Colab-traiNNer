...
class DFNetDataModule(pl.LightningDataModule):
    def __init__(self, dir_lr: str = './',  dir_hr: str = './', val_lr: str = './', val_hr: str = './', batch_size: int = 5, num_workers: int = 2, HR_size = 256, scale = 4, mask_dir: str = './', image_size = 256, amount_tiles = 16, canny_min = 100, canny_max = 150):
        super().__init__()
    ...
    def setup(self, stage=None):
        if cfg['datasets']['train']['mode'] == 'DS_lrhr':
          ...
          self.DFNetdataset_train = VimeoTripletDirect(self.dir_hr)
          self.DFNetdataset_validation = VimeoTriplet_val(self.val_hr)
          self.DFNetdataset_test = VimeoTriplet_val(self.val_hr)
    ...
    def val_dataloader(self):
        return DataLoader(self.DFNetdataset_validation, batch_size=1, num_workers=self.num_workers)
