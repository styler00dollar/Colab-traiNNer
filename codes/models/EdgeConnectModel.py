...
class EdgeConnectInpaintor(BaseModel):
    def __init__(self, opt):
        super(EdgeConnectInpaintor, self).__init__(opt)
    ...
    def optimize_parameters(self, step):
        ...
        with self.cast():
            self.fake_H = self.netG(self.var_L, self.canny_data, self.grayscale_data, mask) #[0] for dfn hotfix
        #/with self.cast():
    ...
    def test(self):
        ...
        with torch.no_grad():
            if self.is_train:
                self.fake_H = self.netG(self.var_L, self.canny_data, self.grayscale_data, mask)
            else:
                self.fake_H = self.netG(self.var_L, self.canny_data, self.grayscale_data, mask, isTest=True)
